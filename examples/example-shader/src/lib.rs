...
fn acos(v: f32) -> f32 {
    let x = builtin_absf32(v);
    let mut res = -0.155972 * x + 1.56467; // p(x)
    res *= builtin_sqrtf32(1.0f32 - x);
...
fn total_rayleigh(lambda: Vec3) -> Vec3 {
    (8.0 * builtin_powf32(PI, 3.0)
        * builtin_powf32(builtin_powf32(REFRACTIVE_INDEX, 2.0) - 1.0, 2.0)
...
fn rayleigh_phase(cos_theta: f32) -> f32 {
    (3.0 / (16.0 * PI)) * (1.0 + builtin_powf32(cos_theta, 2.0))
}
...
fn sky(dir: Vec3, sun_position: Vec3) -> Vec3 {
    ...
    let sunfade = 1.0 - clamp(1.0 - builtin_expf32(sun_position.1 / 450000.0), 0.0, 1.0);
    ...
    let sun_angular_diameter_cos = builtin_cosf32(SUN_ANGULAR_DIAMETER_DEGREES);
