...
fn prebuilt_shaders(options: &Options) -> CompiledShaderModules {
...
fn watch(
    options: &Options,
    mut on_watch: impl FnMut(CompiledShaderModules) + Clone + Send + 'static,
) {
    ...
    // HACK(eddyb) this thread is only needed because `builder.watch` blocks
    // for the first result, but we already have an initial state to use.
    std::thread::spawn(move || {
        let initial_result = builder
            .watch({
                let mut on_watch = on_watch.clone();
                move |compile_result| on_watch(handle_compile_result(compile_result))
            })
            .expect("Configuration is correct for watching");
        on_watch(handle_compile_result(initial_result));
    });
