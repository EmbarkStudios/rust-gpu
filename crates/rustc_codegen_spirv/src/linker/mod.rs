...
pub fn link(
    ...
    // NOTE(eddyb) SPIR-T pipeline is entirely limited to this block.
    {
        ...
        let module = &mut match spirt::Module::lower_from_spv_bytes(cx, spv_bytes) {
            Ok(module) => module,
            Err(e) => {
        ...
        // TODO(eddyb) set the name into `dump_guard` to be able to access it on panic.
...
impl Drop for SpirtDumpGuard<'_> {
    fn drop(&mut self) {
        ...
        if let Some(dump_spirt_file_path) = &dump_spirt_file_path {
            if self.linker_options.spirt_strip_custom_debuginfo_from_dumps {
                for (_, module) in &mut self.per_pass_module_for_dumping {
                    spirt_passes::debuginfo::convert_custom_debuginfo_to_spv(module);
                }
            }
            if !self.linker_options.spirt_keep_debug_sources_in_dumps {
                for (_, module) in &mut self.per_pass_module_for_dumping {
                    ...
                    for sources in debuginfo.source_languages.values_mut() {
                        ...
                        sources.file_contents.insert(
                            self.module.cx_ref().intern(DOTS),
            ...
            // TODO(eddyb) catch panics and tell the user to use `--dump-spirt-passes`
            // (requires passing ???) - may need quieting the panic handler, maybe
            // controlled by a `thread_local!` while the panic handler is global.
            // this could also be used to collect a panic message!
            // TODO(eddyb) try building plans for individual versions first, or
            // maybe repeat `Plan::for_versions` without the last version if initially panicked?
            ...
            if self.any_spirt_bugs {
                let mut note = self
                    .sess
                    .struct_note_without_error("SPIR-T bugs were encountered");
                note.help(format!(
