...
fn trans_struct<'tcx>(cx: &CodegenCx<'tcx>, span: Span, ty: TyAndLayout<'tcx>) -> Word {
    ...
    for i in ty.fields.index_by_increasing_offset() {
        ...
        let field_ty_spv = field_ty.spirv_type(span, cx);
        field_types.push(field_ty_spv);
...
fn trans_intrinsic_type<'tcx>(
    ...
    match intrinsic_type_attr {
        ...
        IntrinsicType::Vector => {
            // Expects a Rust type definition as of `struct VecN<Scalar> { a: Scalar, .. }`.
            let element_kind = substs.type_at(0).kind();
            let element = match element_kind {
                TyKind::Bool => SpirvType::Bool,
                TyKind::Int(int) => match int {
                    IntTy::Isize => {
                        SpirvType::Integer(cx.tcx.data_layout.pointer_size.bits() as u32, true)
                    }
                    IntTy::I8 => SpirvType::Integer(8, true),
                    IntTy::I16 => SpirvType::Integer(16, true),
                    IntTy::I32 => SpirvType::Integer(32, true),
                    IntTy::I64 => SpirvType::Integer(64, true),
                    IntTy::I128 => SpirvType::Integer(128, true),
                },
                TyKind::Uint(uint) => match uint {
                    UintTy::Usize => {
                        SpirvType::Integer(cx.tcx.data_layout.pointer_size.bits() as u32, false)
                    }
                    UintTy::U8 => SpirvType::Integer(8, false),
                    UintTy::U16 => SpirvType::Integer(16, false),
                    UintTy::U32 => SpirvType::Integer(32, false),
                    UintTy::U64 => SpirvType::Integer(64, false),
                    UintTy::U128 => SpirvType::Integer(128, false),
                },
                TyKind::Float(FloatTy::F32) => SpirvType::Float(32),
                TyKind::Float(FloatTy::F64) => SpirvType::Float(64),
                _ => {
                    cx.tcx.sess.span_err(
                        span,
                        &format!("Invalid element type {:?} for `Vector`.", element_kind),
                    );
                    return Err(ErrorReported);
                }
            }
            .def(span, cx);
            let count = ty.fields.count() as u32;
            let spirv_ty = SpirvType::Vector { element, count };
            let spirv_ty_size = match spirv_ty.sizeof(cx) {
                Some(size) => size,
                None => {
                    cx.tcx.sess.span_err(span, "`Vector` must be sized.");
                    return Err(ErrorReported);
                }
            };

            if ty.size != spirv_ty_size {
                cx.tcx.sess.span_err(
                    span,
                    &format!(
                        "Invalid size {:?} for `Vector`, expected {:?}.",
                        ty.size, spirv_ty_size
                    ),
                );
                return Err(ErrorReported);
            }

            Ok(SpirvType::Vector { element, count }.def(span, cx))
        }
    }
}
