...
use rspirv::spirv::{Capability, Dim, ImageFormat, StorageClass, Word};
    ...
    if let TyKind::Adt(adt, substs) = *ty.ty.kind() {
        for attr in parse_attrs(cx, cx.tcx.get_attrs(adt.did)) {
            match attr {
                ...
                SpirvAttribute::Image => return trans_image(cx, span, ty, substs),
                SpirvAttribute::Sampler => {
                    // see SpirvType::sizeof
                    if ty.size != Size::from_bytes(4) {
                        cx.tcx.sess.fatal("#[spirv(sampler)] type must have size 4");
                    }
                    return SpirvType::Sampler.def(span, cx);
                }
...
fn trans_image<'tcx>(
    ...
    if substs_types.len() != 7 {
        cx.tcx.sess.fatal(&format!(
            "#[spirv(image)] may only be used on a type with sampled type, dims, depth, sampled, and format type parameters.
            {:?} has type parameters: {:?} in {:?}",
            ty, substs_types, span
        ))
    }
    let sampled_type = if substs_types[0].is_unit() {
        SpirvType::Void.def(span, cx)
    } else {
        match substs_types[0].kind() {
            TyKind::Int(int_ty) => match int_ty {
                ...
                _ => cx.tcx.sess.fatal(&format!(
                    "Image sample signed int type must be ``i8`, `i16`, `i32`, or `i64`.
                    Found: {:?} in {:?}",
                    substs_types[0], span
                ))
            },
            TyKind::Uint(int_ty) => match int_ty {
                ...
                _ => cx.tcx.sess.fatal(&format!(
                    "Image sample unsigned int type must be ``u8`, `u16`, `u32`, or `u64`.
                    Found: {:?} in {:?}",
                    substs_types[0], span
                ))
            },
            TyKind::Float(float_ty) => match float_ty {
                FloatTy::F32 => SpirvType::Float(32).def(span, cx),
                FloatTy::F64 => SpirvType::Float(64).def(span, cx),
            },
            _ => cx.tcx.sess.fatal(&format!(
                "Image sample type must be `()`, `u8`, `u16`, `u32`, `u64`, `i8`, `i16`, `i32`, `i64`, `f32`, or `f64`.
                Found: {:?} in {:?}",
                substs_types[0], span
            ))
        }
    };
    let eval_const_param = |substs_type: Ty<'tcx>| -> u32 {
        match substs_type.kind() {
            TyKind::Adt(_, substs2) => {
                let consts = substs2.consts().collect::<Vec<_>>();
                if consts.len() != 1 {
                    cx.tcx.sess.fatal(&format!(
                        "Image type parameters must have one const parameter.
                        {:?} has const params: {:?} in {:?}",
                        substs_type, consts, span
                    ))
                };
                consts[0].eval_usize(cx.tcx, cx.param_env()) as u32
            }
            _ => cx.tcx.sess.fatal(&format!(
                "Image const parameters must be ADTs.
                {:?} is kind: {:?} in {:?}",
                substs_type,
                substs_type.kind(),
                span
    ...
    let dim = match Dim::from_u32(eval_const_param(substs_types[1])) {
        Some(dim) => dim,
        None => cx.tcx.sess.fatal(&format!(
            "ImageDim const parameter must be in 0..=6.
            {:?} in {:?}",
            substs_types[0], span
        )),
    };
    let depth = eval_const_param(substs_types[2]);
    let sampled = eval_const_param(substs_types[3]);
    let image_format = match ImageFormat::from_u32(eval_const_param(substs_types[4])) {
        Some(format) => format,
        None => cx.tcx.sess.fatal(&format!(
            "ImageFormat const parameter must be in 0..=41.
            {:?} in {:?}",
            substs_types[4], span
        )),
    };
    let arrayed = eval_const_param(substs_types[5]);
    if arrayed > 1 {
        cx.tcx.sess.fatal(&format!(
            "Arrayed must be 0 for not arrayed or 1 for arrayed.
            {:?} in {:?}",
            substs_types[5], span
        ))
    };
    let multisampled = eval_const_param(substs_types[6]);
    if multisampled > 1 {
        cx.tcx.sess.fatal(&format!(
            "Multisampled must be 0 for not multisampled or 1 for multisampled.
            {:?} in {:?}",
            substs_types[6], span
        ))
    };
